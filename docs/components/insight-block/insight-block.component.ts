...
@Component({
  ...
  template: `
    <div
      class="border border-slate-200 rounded-lg bg-white shadow-sm transition-shadow hover:shadow-md"
    >
      <div class="p-3 cursor-pointer" (click)="toggleExpand()">
        <div class="flex justify-between items-start">
          ...
          <div class="flex items-center gap-2">
            @if (addToSkeleton || removeFromSkeleton) {
            <button
              (click)="onToggleSkeleton($event)"
              [title]="
                isInSkeleton() ? 'Remove from skeleton' : 'Add to skeleton'
              "
            ...
            <button
              class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 text-slate-500"
            >
              <svg
      ...
      <div class="border-t border-slate-200 p-4">
        ...
        <pre
          class="bg-slate-50 p-3 rounded-md text-sm text-slate-700 whitespace-pre-wrap font-sans"
          >{{ block().content }}</pre
        >
        ...
        <div class="mt-4">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <span
              class="px-2 py-0.5 text-xs font-medium bg-slate-100 text-slate-600 rounded"
            >
              {{ tag }}
            </span>
            }
...
export class InsightBlockComponent {
  ...
  blockTypeColor = computed(() => {
    ...
    if (type.includes("conclusion") || type.includes("result"))
      return "bg-green-100 text-green-800";
    if (type.includes("code")) return "bg-gray-200 text-gray-800";
    if (type.includes("kpi") || type.includes("metric"))
